...
export async function uploadImageFile(file) {
  ...
  return { url, previewUrl: localPreview, name: file.name || 'image.jpg' };
}
...
export async function uploadVideoFile(file) {
  ...
  // BỎ withProject: Không gửi project_id lúc upload video tham chiếu
  // để tránh backend tự động gộp nó vào danh sách thư viện video của project
  const payload = {
    file,
    pick: false,
  ...
  console.info(`[project] media.upload_video (không đính kèm project_id)`);
  const res = await bridgeCall('media.upload_video', payload);
  const url = pickUrl(res);
  if (!url) throw new Error('Upload video không thành công');
  
  return { url, previewUrl: localPreview, name: file.name || 'clip.mp4', seconds };
}
