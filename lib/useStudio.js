  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const isValidMediaUrl = (url) =>
        url && !url.startsWith('data:') && !url.includes('tmpfiles.org');

      if (saved.character && isValidMediaUrl(saved.character.url)) {
        setCharacter(saved.character);
      } else if (saved.character) {
        setCharacter(null);
      }
      if (Array.isArray(saved.fashion)) {
        setFashion(saved.fashion.filter((f) => isValidMediaUrl(f?.url)));
      }
      if (Array.isArray(saved.videos)) {
        setVideos(saved.videos.filter((v) => isValidMediaUrl(v?.url)));
      }
