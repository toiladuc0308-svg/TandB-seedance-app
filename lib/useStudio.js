...
export default function useStudio() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (saved.character && !saved.character?.url?.startsWith('data:')) {
        setCharacter(saved.character);
      }
      if (Array.isArray(saved.fashion)) {
        setFashion(saved.fashion.filter((f) => f?.url && !f.url.startsWith('data:')));
      }
      if (Array.isArray(saved.videos)) {
        setVideos(saved.videos.filter((v) => v?.url && !v.url.startsWith('data:')));
      }
