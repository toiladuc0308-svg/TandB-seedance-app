...
export function pickDurationForSeconds(options, seconds) {
  ...
  console.log(`[DEBUG_DURATION] Before rounding: ${target}s -> After Math.ceil: ${roundedTarget}s`);

  const ceilOption = list.find((o) => o.seconds >= roundedTarget);
  if (ceilOption) return ceilOption.type;
