...
export async function runScene({
  ...
  const refUrls = [scene.characterUrl, scene.fashion?.url].filter(Boolean);
  const vUrls = [scene.video?.url].filter(Boolean);

  for (const u of [...refUrls, ...vUrls]) {
    if (typeof u === 'string' && u.startsWith('data:')) {
      throw new Error(
        'Ảnh hoặc video đang ở dạng dữ liệu cục bộ (base64) chưa được tải lên máy chủ 79AI. Vui lòng bấm "Dán link" để nhập URL online (https://...) hoặc tải lại ảnh.'
      );
    }
    if (typeof u === 'string' && !u.startsWith('http://') && !u.startsWith('https://')) {
      throw new Error(
        `URL media không hợp lệ: "${u}". Vui lòng cung cấp link URL hợp lệ (bắt đầu bằng https://).`
      );
    }
  }
  ...
  console.log('[DEBUG_RATIO_2] engine.js: effectiveSettings passed to API:', JSON.stringify(effectiveSettings));
  console.log('[DEBUG_RATIO_2] engine.js: body sent to API:', JSON.stringify(body));
  ...
  try {
    ...
    console.log('[DEBUG_RATIO_3] engine.js: API Response created:', JSON.stringify(created));
  } catch (error) {
